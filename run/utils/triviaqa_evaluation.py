...
def metric_max_over_ground_truths(metric_fn, prediction, ground_truths, **kwargs):
    scores_for_ground_truths = []

    for gt in ground_truths:
        if metric_fn.__name__ == "score":  # bertscore
            # Wrap each input in a list
            P, R, F1 = metric_fn([prediction], [gt], **kwargs)
            score = float(F1[0].item())  # scalar F1
        else:
